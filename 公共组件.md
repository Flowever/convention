...
## 安装
```shell
pod repo add FECommonSpecs git@git.flowever.net:Flowever/Common.git
```
在 podfile 中加入
```ruby
source 'https://github.com/CocoaPods/Specs.git'
pod 'Common', :git => 'http://git.flowever.net/Flowever/Common.git', :branch => '0.0.4'
```
然后执行
```shell
pod install
...
showAlert(action: ((_ ok: Bool) -> Void)? = nil)
